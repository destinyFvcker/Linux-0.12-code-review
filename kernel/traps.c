...
#include <asm/io.h>				// 输入/输出头文件。定义硬件端口输出/输出宏汇编语句

// 以下语句定义了三个嵌入式汇编宏语句函数。 
// 用圆括号括住的复合语句（花括号中的语句）可以作为表达式使用，其中最后的__res 是其输出值。 
// 在下面这个宏定义函数块之中定义了一个寄存器变量__res。该变量将被保存在一个寄存器中，以便
// 于快速访问和操作。 
// 如果想指定寄存器（例如 eax），那么我们可以把该句写成“register char __res asm("ax");”。 
// 
// 取段 seg 中地址 addr 处（实际上就是一个偏移量）的一个字节。 
...
// 下面定义了一些函数原型
void page_exception(void);				// 页异常。实际上是page_fault
...
static void die(char * str,long esp_ptr,long nr)
{
	...
	printk("%s: %04x\n\r",str,nr&0xffff);	// 这里只是取了nr的低16位
// 下行打印语句显示当前调用进程的 CS:EIP、EFLAGS 和 SS:ESP 的值。这里 esp[0] 
...
// 下面吧int17-47的陷阱门均线设置位reserved，以后各个硬件初始化的时候会重新设置自己的陷阱门
	for (i=18;i<48;i++)
